...
INSERT INTO subsidy_programs_extended (
  ...
  '{
    "business_type": ["中小企業", "小規模事業者"],
    "innovation_focus": ["革新的サービス開発", "試作品開発", "生産プロセス改善"],
    "investment_type": ["設備投資", "システム開発"]
  }',
  '{
    "innovation": {"weight": 30, "description": "革新性・独自性"},
    "technical_feasibility": {"weight": 25, "description": "技術的実現可能性"},
    "market_potential": {"weight": 20, "description": "市場性・収益性"},
    "execution_capability": {"weight": 15, "description": "実行体制・能力"},
    "policy_alignment": {"weight": 10, "description": "政策的意義"}
  }',
  '[
    {"category": "company_basic", "priority": 1},
    {"category": "business_model", "priority": 2},
    {"category": "innovation_plan", "priority": 3},
    {"category": "technical_details", "priority": 4},
    {"category": "market_strategy", "priority": 5},
    {"category": "financial_plan", "priority": 6}
  ]'
);

-- ものづくり補助金用質問テンプレート
INSERT INTO question_templates (subsidy_id, category, question_text, question_type, is_required, ai_context) 
SELECT 
  s.id,
  'company_basic',
  '会社の主力事業・製品・サービスを具体的に教えてください。',
  'textarea',
  true,
  '申請者の事業概要を把握し、補助事業との関連性を評価するための基本情報'
FROM subsidy_programs_extended s WHERE s.type = 'monozukuri';

INSERT INTO question_templates (subsidy_id, category, question_text, question_type, is_required, ai_context)
SELECT 
  s.id,
  'innovation_plan',
  '今回の補助事業で実現したい革新的な取り組みを教えてください。',
  'textarea',
  true,
  '補助事業の革新性と独自性を評価するための核心的な質問'
FROM subsidy_programs_extended s WHERE s.type = 'monozukuri';
...
SELECT 
  s.id,
  'technical_details',
  '導入予定の設備・システムの技術的詳細を教えてください。',
  'textarea',
  true,
  '技術的実現可能性と設備投資の妥当性を評価'
FROM subsidy_programs_extended s WHERE s.type = 'monozukuri';

-- 申請書テンプレート（ものづくり補助金）
INSERT INTO application_templates (subsidy_id, section_name, section_order, content_template, ai_prompts)
SELECT 
  s.id,
  '事業計画の具体性・実現可能性',
  1,
